...
- [x] 5.4 Unificar los estados de deshabilitado —opacidad, cursor y atributo semántico— con un token, no con valores por componente. — Había tres opacidades (0.4, 0.5, 0.6) en siete reglas; ahora `--disabled-opacity`. La parte semántica destapó que `.drop-zone` era un `<div>` con `@click` **sin manejador de teclado**: subir un archivo era imposible sin ratón.
