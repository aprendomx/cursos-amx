...
const aggregate = computed(() => {
  ...
  const abandonos = Math.round(
    stats.value.reduce((sum, r) => sum + (r.total_vistas_unicas ?? 0) * ((r.tasa_abandono_pct ?? 0) / 100), 0)
  )
    ...
    <template v-else>
      ...
      >
        <div class="card" :style="{ padding: 'calc(var(--unit) * 3)' }">
          <p class="caption" :style="{ color: 'var(--text-secondary)', marginBottom: 'calc(var(--unit) * 1)' }">
            Vistas únicas
          </p>
          <p class="h4">{{ aggregate.vistasUnicas }}</p>
        </div>
        <div class="card" :style="{ padding: 'calc(var(--unit) * 3)' }">
          <p class="caption" :style="{ color: 'var(--text-secondary)', marginBottom: 'calc(var(--unit) * 1)' }">
        ...
        <div class="card" :style="{ padding: 'calc(var(--unit) * 3)' }">
          ...
          <p class="h4">{{ aggregate.abandonos }}</p>
        </div>
        <div class="card" :style="{ padding: 'calc(var(--unit) * 3)' }">
          <p class="caption" :style="{ color: 'var(--text-secondary)', marginBottom: 'calc(var(--unit) * 1)' }">
            Tiempo total visto
          </p>
          <p class="h4">{{ formatearDuracion(aggregate.watchTimeSeg) }}</p>
        </div>
      </div>
              ...
              <td class="mono" :style="{ textAlign: 'right' }">
                {{
                  row.total_vistas_unicas
                    ? formatearDuracion(Math.round((row.total_segundos_vistos ?? 0) / row.total_vistas_unicas))
                    : '—'
                }}
              </td>
              <td :style="{ textAlign: 'right' }">
                <span
                  class="mono"
                  :style="{ color: (row.tasa_completitud_pct ?? 100) < 50 ? 'var(--danger)' : 'var(--ink-2)' }"
                >
                  {{ row.tasa_completitud_pct != null ? row.tasa_completitud_pct.toFixed(1) + '%' : '—' }}
                </span>
              </td>
              <td :style="{ textAlign: 'right' }">
                <span
                  class="mono"
                  :style="{ color: (row.tasa_abandono_pct ?? 0) > 30 ? 'var(--danger)' : 'var(--ink-2)' }"
                >
