// supabase/functions/analytics/handler.ts
// Edge Function para reportes administrativos avanzados (Fase H1).
// Acciones: reporte_csv, funnel, retencion, comparativa.
// Requiere rol admin; los instructores solo pueden usar INSTRUCTOR_ACTIONS
// y su instructor_id se deriva siempre del token, no del body.
...
export function createHandler(clientFactory: () => any) {
  ...
  try {
    ...
    const auth = await authorize(req, supabase, { anyOf: ['admin', 'instructor'] })
    if (!auth.ok) return authErrorResponse(auth)
    const isAdmin = auth.roles?.es_admin === true

    const body = await req.json()
    const { action } = body

    if (!isAdmin && !INSTRUCTOR_ACTIONS.includes(action)) {
    ...
    switch (action) {
      ...
      case 'instructor_dashboard': {
        // Un instructor solo ve su propio dashboard; el admin puede consultar otros.
        const instructor_id =
          isAdmin && body.instructor_id ? body.instructor_id : auth.user!.id
        const { data, error } = await supabase
