// supabase/functions/video-analytics/handler.ts
...
/**
 * Un usuario solo puede reportar eventos propios; un admin puede reportar
 * de cualquiera. Los eventos ajenos se descartan como errores.
 */
export function enforceOwnership(
  ...
  if (isAdmin) return { events, errors: [] }

  const own: VideoEventInput[] = []
  const errors: string[] = []
  for (let i = 0; i < events.length; i++) {
    if (events[i].user_id === callerId) {
      own.push(events[i])
    } else {
      errors.push(`Evento ${i}: user_id no coincide con el usuario autenticado`)
    }
  }
  return { events: own, errors }
}
...
export function createHandler(clientFactory: () => any) {
  ...
  const owned = enforceOwnership(
    parsed.events,
    auth.user!.id,
    auth.roles?.es_admin === true
  )
  const events = owned.events
  const errors = [...parsed.errors, ...owned.errors]
